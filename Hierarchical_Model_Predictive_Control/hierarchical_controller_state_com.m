...
Q = Ex'*Qy*Ex;
R = diag([0.1, 0.1]);
model_s.u.penalty = QuadFunction(R);
model_s.x.penalty = QuadFunction(Q);
...
for j = 0:Nsim_s-1
    ...
    tic
        ...
        r(1) = theta1_d(round((i+j*steps_ratio)*Ts/0.01)); %desired motor angles
        r(2) = theta2_d(round((i+j*steps_ratio)*Ts/0.01));
        r(1) = sin((i+j*steps_ratio)*0.03);
        r(2) = r(1);
        %try
            if i==1 %|| i==steps_ratio
                r_s = [0;0];
                r_s(1) = theta1_d(i+(j+1)*steps_ratio);
                r_s(2) = theta1_d(i+(j+1)*steps_ratio);
                ...
                ctrl.model.x.min=[-inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf];
                ctrl.model.x.max=-ctrl.model.x.min;
                
                
                %ctrl.model.x.min = [data.X_s(:,j+1)', data.U_s(:,j+1)', -inf, -inf]-0.1*abs([data.X_s(:,j+1)', data.U_s(:,j+1)', -inf, -inf]);
                %ctrl.model.x.max = [data.X_s(:,j+1)', data.U_s(:,j+1)', -inf, -inf]+0.1*abs([data.X_s(:,j+1)', data.U_s(:,j+1)', -inf, -inf]);
            elseif i==steps_ratio
                %ctrl.model.x.min=[-inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf];
                %ctrl.model.x.max=-ctrl.model.x.min;
                ctrl.model.x.min = [data.X_s(:,j+1)', data.U_s(:,j+1)', -inf, -inf]-0.01*ones(1,8);
                ctrl.model.x.max = [data.X_s(:,j+1)', data.U_s(:,j+1)', inf, inf]+0.01*ones(1,8);
            else
                ctrl.model.x.min=[-inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf];
                ctrl.model.x.max=-ctrl.model.x.min;
